    ...
    dcc.Tabs([
        ...
        dcc.Tab(children=[  # Fourth tab - Average score per numeric column for the selected lesson through the years
            dcc.Dropdown(
                ...
                options=[
                    {'label': i, 'value': y} for i, y in config.dict_median_columns
                ],
...
def first_tab(courseid, syear):
    ...
    piechart1 = px.pie(first_tab_df, names='q1_attendslectures', hole=.0, title='Παρακολούθησαν το μάθημα')
    piechart2 = px.pie(first_tab_df, names='q2b_attendslabs', hole=.0, title='Παρακολουθεί τα εργαστήρια')
    barchart = px.bar(first_tab_df, x="q14_evaluationcriteria", title="Κριτήρια αξιολόγησης")
...
def second_tab(courseid, syear):
    ...
    piechart = px.pie(second_tab_df, names='q2_coursehaslabs', hole=.0, title='Έχει το μάθημα εργαστήρια')
...
def third_tab(courseid, syear):
    ...
    piechart = px.pie(third_tab_df, names='q11_hasassistant', hole=.0, title='Έχει βοηθό')
...
def fourth_tab(courseid, median_column):
    ...
    barchart = px.line(fourth_tab_df[config.median_columns_plus_year].groupby(['qyear']).mean(), y=median_column,
